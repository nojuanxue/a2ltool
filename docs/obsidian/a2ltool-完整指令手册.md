...
5. 点击“保存为默认配置”，可将当前全部 GUI 设置保存为下次启动自动加载的默认配置；
6. 界面已启用统一字体和响应式布局，窗口放大/缩小时内部控件会跟随调整；
7. “命令预览”区域支持拖动分隔条上下调节高度，方便在小屏时优先看日志或预览；
   - 分隔条带可视化拖拽手柄，并放宽了最小高度限制，可把预览区域继续向上收起；
8. 界面最右侧提供全局纵向滚动条，可上下移动查看所有内容；
9. 运行时关注状态栏（就绪/运行中/运行成功/运行失败），执行完成后会弹窗提示结果；
10. 把 `.rsp` 纳入版本管理，CI 直接调用。
